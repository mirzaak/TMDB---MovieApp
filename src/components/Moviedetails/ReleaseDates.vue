  ...
  <div class="pageWrapper">
      <div class="main">
          <div class="left">
              <div class="panel">
                  <div class="blueBox">
                      <h3>Release Dates</h3><p>{{otherData.length}}</p>                  </div>
                  <div @click="scrollToElement(b.iso_3166_1)" class="panelContent" v-for="b in otherData" :key="b" ref="b">
                      <span ><a>{{b.iso_3166_1}}</a><p></p></span>
                  </div>
              </div>
          </div>
          <div class="right">
                      ...
                      <div class="up"><a ref="a">{{a.iso_3166_1}}</a></div>
                        <div class="down">
                            <div class="downUpWrap">
                                <div class="downUp"><a>Date</a><a>Classification</a><a>Type</a><a>Language</a><a>Note</a></div>
                            </div>
                            <div v-for="index in a.release_dates" :key="index" class="downDown" id="element">
                            <a>{{index.release_date.toString().substring(0,10)}}</a><a>{{index.certification}}</a><a>{{index.type}}</a><a>/</a><a>{{index.note}}</a>
                            </div>
                        </div>
                  </div>
              </div>
...
import { ref, shallowRef } from 'vue'
    ...
    try{
      let d = await axios.get('https://api.themoviedb.org/3/movie/'+id+'/release_dates?api_key=0b5e8ce7494ae54d6c643adf4db40da7')
      otherData.value = await d.data.results
      rDates.value = await d.data.results.release_dates
    console.log(otherData.value)
    console.log(rDates.value)
